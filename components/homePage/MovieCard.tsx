...
const MovieCard: React.FC<any> = ({ movie ,type }) => {
  return (
    <Link href={`/${type}/`+movie.id} className="ml-5 hover:ml-0" >
      <div className="relative w-40 movieCard h-60 hover:w-48 hover:h-64 hover:mt-0 mt-6   cursor-pointer overflow-hidden hover:border hover:border-gray-800">
        ...
        <div className="relative  px-3 movieCardDescription w-full h-full pt-36 bg-gradient-to-t from-black to-[rgba(5,5,5,.5)]">
          <div className=" h-fit ralative bottom-0">
            <div className="">
              <p className="text-sm">{type==='movies'? movie.original_title : movie.name}</p>
      ...
      <div className="text-xs mt-2">{type==='movies'? movie.original_title : movie.name}</div>
      <div className="text-xs text-gray-400">{type==='movies'?movie.release_date :movie.first_air_date}</div>
    </Link>
  );
};
